...
def cursky():
    response = requests.get("https://graph.talparad.io/?query=query+GetPlayouts%28%24slug%3AString%21%24nextToken%3AString%24startDate%3AAWSDateTime%29%7Bstation%28slug%3A%24slug%29%7BgetPlayouts%28nextToken%3A%24nextToken+limit%3A100+broadcastStartDate%3A%24startDate%29%7Bplayouts%7Bid+broadcastDate+track%7Bisrc+id+title+artistName+media%7Bid+source+type+title+uri%7Dimages%7Btitle+uri%7D%7D%7DnextToken%7D%7D%7D&variables=%7B%22slug%22%3A%22sky-radio%22%2C%22nextToken%22%3Anull%2C%22startDate%22%3A%222026-04-16T12%3A06%3A57.420Z%22%2C%22skip%22%3A100%7D", headers={"x-api-key": "da2-abza7qpnqbfe5ihpk4jhcslpgy"})
    r = json.loads(response.content)
    #print(r)

    dingen = r["data"]["station"]["getPlayouts"]["playouts"]
    t = dingen[0]
...
def curradio1():
    ...
    # de class selctor is hier van de div die alle losse blokken heeft, niet het eerste blok. (die blokken zijn namelijk soms divs, en soms a, dus kan je niet op selecten)
    t = soep.select(".bXtIGO > *")[0]
